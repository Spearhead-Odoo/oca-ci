...
ARG GIT_TOKEN=""
# Additional repositories to clone (space-separated list: "owner/repo:folder owner2/repo2:folder2")
...
ADD https://api.github.com/repos/$odoo_org_repo/git/refs/heads/$odoo_version /tmp/branch.json

# Use the commit SHA from JSON to download exact requirements.txt
RUN SHA=$(jq -r .object.sha /tmp/branch.json) \
 && curl -sSL "https://raw.githubusercontent.com/$odoo_org_repo/${SHA}/requirements.txt" \
...
# Clone additional repositories dynamically
RUN if [ -n "${ADDITIONAL_REPOS}" ]; then \
      echo "Cloning additional repositories: ${ADDITIONAL_REPOS}" && \
      for repo_info in ${ADDITIONAL_REPOS}; do \
        repo=$(echo "$repo_info" | cut -d: -f1) && \
        folder=$(echo "$repo_info" | cut -d: -f2) && \
        echo "Cloning $repo into /opt/odoo/$folder" && \
        git clone --depth 1 --branch $odoo_version https://${GIT_TOKEN}@github.com/$repo.git /opt/odoo/$folder; \
      done; \
    else \
      echo "No additional repositories to clone"; \
    fi
...
# Update ADDONS_PATH in oca_install_addons script only if additional repositories were cloned
RUN if [ -n "${ADDITIONAL_REPOS}" ] && [ "${ADDITIONAL_REPOS}" != "" ]; then \
      ADDONS_PATH_BUILD="/opt/odoo/addons" && \
      for repo_info in ${ADDITIONAL_REPOS}; do \
        folder=$(echo "$repo_info" | cut -d: -f2) && \
        ADDONS_PATH_BUILD="$ADDONS_PATH_BUILD,/opt/odoo/$folder"; \
      done && \
      echo "Setting ADDONS_PATH in script to: $ADDONS_PATH_BUILD" && \
      sed -i "s|\${ADDONS_PATH}|$ADDONS_PATH_BUILD|g" /usr/local/bin/oca_install_addons; \
    else \
      echo "No additional repositories configured, keeping original ADDONS_PATH behavior"; \
    fi
